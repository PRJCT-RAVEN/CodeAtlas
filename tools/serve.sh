...
#   0  viewer already answering on :5173, the port is held by something else, or the checkout
#      cannot be read — nothing to do, do NOT relaunch (this is what stops a relaunch storm)
...
# Readability comes first, before any work that could fail on its own: a checkout under
# ~/Documents, ~/Desktop or ~/Downloads is invisible to a launchd-spawned process (TCC),
# and so is one that has been moved or renamed. Every path out of this script would then
# exit non-zero under `set -eu` — which is precisely what KeepAlive relaunches, every
# ThrottleInterval, forever, with nothing but log lines to show for it.
HERE="$(cd "$(dirname "$0")" 2>/dev/null && pwd)" || HERE=""
if [ -z "$HERE" ] || [ ! -r "$HERE/../viewer/package.json" ]; then
  log "cannot read $(dirname "$0")/../viewer — TCC-protected folder (~/Documents, ~/Desktop, ~/Downloads) or a moved checkout; not starting. Move the checkout and reinstall the agent (see CLAUDE.md)."
  exit 0
fi
